    ...
    */
    ...
    public class ClientWsTratamientoAbono
    {
        ...
        public List<Tratamientodental> ListarTratamientoIdFicha(int idFicha)
        {
            ...
            try
            {
                String result = netclient.NetPost("http://"+ipServer+"/proyectoTitulo/sfhwebservice/webService/ws-tratamiento-abono.php", "send={\"indice\":4,\"idFicha\":"+idFicha+"}");
        ...
        public List<Abono> ListarAbonoPorTratamiento(int idTratamiento)
        {
            ...
            try
            {
                String result = netclient.NetPost("http://"+ipServer+"/proyectoTitulo/sfhwebservice/webService/ws-ficha-presupuesto.php",jsonAEnviar);
                var jobject = JObject.Parse(result);
        ...
        public string InsertarAbono(Abono abono)
        {
            //{"indice":5,"idTratamientoDental":3,"fechaAbono":"1991-12-12","monto":1000}
            string jsonAEnviar = "send={\"indice\":5,\"idTratamientoDental\":"+abono.IdTratamientoDental+",\"fechaAbono\":\""+abono.FechaAbono+"\",\"monto\":"+abono.Monto+"}";
            try
            {
                String result = netclient.NetPost("http://"+ipServer+"/proyectoTitulo/sfhwebservice/webService/ws-ficha-presupuesto.php", jsonAEnviar);
                var jobject = JObject.Parse(result);
                //{"code":1,"idTratamientoInsertada":10}
                return jobject.SelectToken("idAbonoInsertado").ToString();
            }
            catch (Exception e)
            {
                throw new Exception(e + "| Error al Listar Fichas");
        ...
        public string ModificarAbono(Abono abono)
        {
            ...
            string jsonAEnviar = "send={\"indice\":6,\"idAbono\":"+abono.IdAbono+",\"idTratamientoDental\":"+abono.IdTratamientoDental+",\"fechaAbono\":\""+abono.FechaAbono+"\",\"monto\":"+abono.Monto+"}";
            try
            {
                String result = netclient.NetPost("http://"+ipServer+"/proyectoTitulo/sfhwebservice/webService/ws-ficha-presupuesto.php", jsonAEnviar);
                var jobject = JObject.Parse(result);
